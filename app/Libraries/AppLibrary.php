...
class AppLibrary
{
    ...
    public static function numericToAssociativeArrayBuilder($array): array
    {
        ...
        if (count($array)) {
            // First pass: collect all parents (parent == 0) and map their IDs to their buildArray index
            foreach ($array as $arr) {
                if (!$arr['parent']) {
                    $parentMap[$arr['id']] = $i;
                    $buildArray[$i] = $arr;
            ...
            // Second pass: associate children with their parents regardless of processing order
            foreach ($array as $arr) {
                if ($arr['parent'] && isset($parentMap[$arr['parent']])) {
                    $buildArray[$parentMap[$arr['parent']]]['children'][] = $arr;
                }
            }
        }
        ...
        return array_values($buildArray); // Re-index to ensure it's a numeric array for Vue resources
    }
