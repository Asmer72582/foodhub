<?php

namespace App\Services;

use Exception;
use Carbon\Carbon;
use App\Models\Item;
use App\Models\User;
use App\Models\Order;
use App\Enums\OrderStatus;
use App\Enums\PaymentStatus;
use App\Enums\PaymentMethod;
use Illuminate\Http\Request;
use App\Libraries\AppLibrary;
use App\Enums\Role as EnumRole;
use Illuminate\Support\Facades\Log;
use Illuminate\Support\Facades\DB;
use App\Enums\PosPaymentMethod;

class DashboardService
{
    /**
     * @throws Exception
     */
    public function orderStatistics(Request $request)
    {
        try {
            $order = new Order;

            if ($request->first_date && $request->last_date) {
                $first_date = Date('Y-m-d', strtotime($request->first_date));
                $last_date  = Date('Y-m-d', strtotime($request->last_date));
            } else {
                $first_date = Carbon::today()->toDateString();
                $last_date  = Carbon::today()->toDateString();
            }

            $orderStatisticsArray = [];

            $orderStatisticsArray["total_order"]            = $order->whereDate('order_datetime', '>=', $first_date)->whereDate('order_datetime', '<=', $last_date)->count();
            $orderStatisticsArray["pending_order"]          = $order->pending()->whereDate('order_datetime', '>=', $first_date)->whereDate('order_datetime', '<=', $last_date)->count();
            $orderStatisticsArray["processing_order"]       = $order->processing()->whereDate('order_datetime', '>=', $first_date)->whereDate('order_datetime', '<=', $last_date)->count();
            $orderStatisticsArray["out_for_delivery_order"] = $order->outForDelivery()->whereDate('order_datetime', '>=', $first_date)->whereDate('order_datetime', '<=', $last_date)->count();
            $orderStatisticsArray["delivered_order"]        = $order->delivered()->whereDate('order_datetime', '>=', $first_date)->whereDate('order_datetime', '<=', $last_date)->count();
            $orderStatisticsArray["canceled_order"]         = $order->canceled()->whereDate('order_datetime', '>=', $first_date)->whereDate('order_datetime', '<=', $last_date)->count();
            $orderStatisticsArray["returned_order"]         = $order->returned()->whereDate('order_datetime', '>=', $first_date)->whereDate('order_datetime', '<=', $last_date)->count();
            $orderStatisticsArray["rejected_order"]         = $order->rejected()->whereDate('order_datetime', '>=', $first_date)->whereDate('order_datetime', '<=', $last_date)->count();

            return $orderStatisticsArray;
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
            throw new Exception($exception->getMessage(), 422);
        }
    }


    public function orderSummary(Request $request)
    {
        try {
            $order = new Order;
            if ($request->first_date && $request->last_date) {
                $first_date = Date('Y-m-d', strtotime($request->first_date));
                $last_date  = Date('Y-m-d', strtotime($request->last_date));
            } else {
                $first_date = Date('Y-m-01', strtotime(Carbon::today()->toDateString()));
                $last_date  = Date('Y-m-t', strtotime(Carbon::today()->toDateString()));
            }

            $orderSummaryArray = [];

            $total_order   = $order->whereDate('order_datetime', '>=', $first_date)->whereDate('order_datetime', '<=', $last_date)->count();
            $total_delivered = $order->delivered()->whereDate('order_datetime', '>=', $first_date)->whereDate('order_datetime', '<=', $last_date)->count();
            $total_canceled  = $order->canceled()->whereDate('order_datetime', '>=', $first_date)->whereDate('order_datetime', '<=', $last_date)->count();
            $total_returned  = $order->returned()->whereDate('order_datetime', '>=', $first_date)->whereDate('order_datetime', '<=', $last_date)->count();
            $total_rejected  = $order->rejected()->whereDate('order_datetime', '>=', $first_date)->whereDate('order_datetime', '<=', $last_date)->count();


            if ($total_order > 0) {
                $orderSummaryArray["delivered"] = (int) round(($total_delivered * 100) / $total_order);
                $orderSummaryArray["returned"]  = (int) round(($total_returned * 100) / $total_order);
                $orderSummaryArray["canceled"] = (int) round(($total_canceled * 100) / $total_order);
                $orderSummaryArray["rejected"] = (int) round(($total_rejected * 100) / $total_order);
            } else {
                $orderSummaryArray["delivered"] = 0;
                $orderSummaryArray["returned"]  = 0;
                $orderSummaryArray["canceled"] = 0;
                $orderSummaryArray["rejected"] = 0;
            }

            return $orderSummaryArray;
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
            throw new Exception($exception->getMessage(), 422);
        }
    }

    public function salesSummary(Request $request)
    {
        $order = new Order;
        if ($request->first_date && $request->last_date) {
            $first_date = Date('Y-m-d', strtotime($request->first_date));
            $last_date  = Date('Y-m-d', strtotime($request->last_date));
        } else {
            $first_date = Date('Y-m-01', strtotime(Carbon::today()->toDateString()));
            $last_date  = Date('Y-m-t', strtotime(Carbon::today()->toDateString()));
        }

        $date = date_diff(date_create($first_date), date_create($last_date), false);
        $date_diff = (int)$date->format("%a");

        $total_sales     = AppLibrary::flatAmountFormat($order->whereDate('order_datetime', '>=', $first_date)->whereDate('order_datetime', '<=', $last_date)->where('payment_status', PaymentStatus::PAID)->sum('total'));
        $total_tips      = AppLibrary::flatAmountFormat($order->whereDate('order_datetime', '>=', $first_date)->whereDate('order_datetime', '<=', $last_date)->where('payment_status', PaymentStatus::PAID)->sum('tip_amount'));

        $dateRangeArray = [];
        for ($currentDate = strtotime($first_date); $currentDate <= strtotime($last_date); $currentDate += (86400)) {

            $date = date('Y-m-d', $currentDate);
            $dateRangeArray[] = $date;
        }

        $dateRangeValueArray = [];
        $dateRangeTipValueArray = [];
        for ($i = 0; $i <= count($dateRangeArray) - 1; $i++) {
            $per_day     = AppLibrary::flatAmountFormat($order->whereDate('order_datetime', $dateRangeArray[$i])->where('payment_status', PaymentStatus::PAID)->sum('total'));
            $dateRangeValueArray[] = floatval($per_day);
            
            $per_day_tip = AppLibrary::flatAmountFormat($order->whereDate('order_datetime', $dateRangeArray[$i])->where('payment_status', PaymentStatus::PAID)->sum('tip_amount'));
            $dateRangeTipValueArray[] = floatval($per_day_tip);
        }


        $salesSummaryArray = [];
        if ($date_diff > 0) {
            $salesSummaryArray['total_sales']   = AppLibrary::currencyAmountFormat($total_sales);
            $salesSummaryArray['total_tips']    = AppLibrary::currencyAmountFormat($total_tips);
            $salesSummaryArray['avg_per_day']   = AppLibrary::currencyAmountFormat($total_sales / $date_diff);
            $salesSummaryArray['per_day_sales'] = $dateRangeValueArray;
            $salesSummaryArray['per_day_tips']  = $dateRangeTipValueArray;
        } else {
            $salesSummaryArray['total_sales']   = AppLibrary::currencyAmountFormat($total_sales);
            $salesSummaryArray['total_tips']    = AppLibrary::currencyAmountFormat($total_tips);
            $salesSummaryArray['avg_per_day']   = AppLibrary::currencyAmountFormat($total_sales);
            $salesSummaryArray['per_day_sales'] = $dateRangeValueArray;
            $salesSummaryArray['per_day_tips']  = $dateRangeTipValueArray;
        }

        return $salesSummaryArray;
    }

    public function customerStates(Request $request)
    {
        $order = new Order;
        if ($request->first_date && $request->last_date) {
            $first_date = Date('Y-m-d', strtotime($request->first_date));
            $last_date  = Date('Y-m-d', strtotime($request->last_date));
        } else {
            $first_date = Date('Y-m-01', strtotime(Carbon::today()->toDateString()));
            $last_date  = Date('Y-m-t', strtotime(Carbon::today()->toDateString()));
        }

        $timeArray = ["06:00", "07:00", "08:00", "09:00", "10:00", "11:00", "12:00", "13:00", "14:00", "15:00", "16:00", "17:00", "18:00", "19:00", "20:00", "21:00", "22:00", "23:00"];

        $customerSateArray = [];
        $totalCustomerArray = [];
        $first_time = "";
        $last_time = "";
        for ($i = 0; $i <= count($timeArray) - 1; $i++) {
            $first_time = date('H:i', strtotime($timeArray[$i]));
            $last_time = date('H:i', strtotime($timeArray[$i] . ' +59 minutes'));

            $total_customer     = $order->whereDate('order_datetime', '>=', $first_date)->whereDate('order_datetime', '<=', $last_date)->whereTime('order_datetime', '>=', Carbon::parse($first_time))->whereTime('order_datetime', '<=', Carbon::parse($last_time))->get()->count();
            $totalCustomerArray[] = $total_customer;
        }

        $customerSateArray['total_customers'] = $totalCustomerArray;
        $customerSateArray['times'] = $timeArray;

        return $customerSateArray;
    }

    public function topCustomers()
    {
        try {
            return User::withCount('orders')->orderBy('orders_count', 'desc')->role(EnumRole::CUSTOMER)->limit(8)->get();
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
            throw new Exception($exception->getMessage(), 422);
        }
    }

    public function employeeOrders(Request $request)
    {
        try {
            if ($request->date_range === 'custom' && $request->start_date && $request->end_date) {
                $first_date = Carbon::parse($request->start_date)->startOfDay();
                $last_date = Carbon::parse($request->end_date)->endOfDay();
            } else {
                switch ($request->date_range) {
                    case 'today':
                        $first_date = Carbon::today()->startOfDay();
                        $last_date = Carbon::today()->endOfDay();
                        break;
                    case 'this_week':
                        $first_date = Carbon::now()->startOfWeek();
                        $last_date = Carbon::now()->endOfWeek();
                        break;
                    case 'this_year':
                        $first_date = Carbon::now()->startOfYear();
                        $last_date = Carbon::now()->endOfYear();
                        break;
                    case 'last_month':
                        $first_date = Carbon::now()->subMonth()->startOfMonth();
                        $last_date = Carbon::now()->subMonth()->endOfMonth();
                        break;
                    case 'this_month':
                    default:
                        $first_date = Carbon::now()->startOfMonth();
                        $last_date = Carbon::now()->endOfMonth();
                        break;
                }
            }

            $users = User::whereHas('roles', function ($query) {
                $query->whereIn('id', [EnumRole::POS_OPERATOR, EnumRole::WAITER, EnumRole::CHEF, EnumRole::BRANCH_MANAGER, EnumRole::STUFF]);
            })
            ->withCount(['createdOrders as total_orders' => function($query) use ($first_date, $last_date) {
                $query->whereBetween('order_datetime', [$first_date->format('Y-m-d H:i:s'), $last_date->format('Y-m-d H:i:s')])
                        ->where('status', '!=', OrderStatus::CANCELED)
                        ->where('status', '!=', OrderStatus::REJECTED);
            }])
            ->withSum(['createdOrders as total_sales' => function($query) use ($first_date, $last_date) {
                $query->whereBetween('order_datetime', [$first_date->format('Y-m-d H:i:s'), $last_date->format('Y-m-d H:i:s')])
                        ->where('payment_status', PaymentStatus::PAID)
                        ->where('status', '!=', OrderStatus::CANCELED)
                        ->where('status', '!=', OrderStatus::REJECTED);
            }], 'total')
            ->withSum(['tippedOrders as total_tips' => function($query) use ($first_date, $last_date) {
                $query->whereBetween('order_datetime', [$first_date->format('Y-m-d H:i:s'), $last_date->format('Y-m-d H:i:s')])
                        ->where('payment_status', PaymentStatus::PAID)
                        ->where('status', '!=', OrderStatus::CANCELED)
                        ->where('status', '!=', OrderStatus::REJECTED);
            }], 'tip_amount')
            ->orderBy('total_orders', 'desc')
            ->get();

            // Format total_sales with currency amount format for API
            $users->transform(function ($user) {
                $user->total_sales_formatted = AppLibrary::currencyAmountFormat($user->total_sales ?? 0);
                $user->total_tips_formatted = AppLibrary::currencyAmountFormat($user->total_tips ?? 0);
                return $user;
            });

            return $users;
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
            throw new Exception($exception->getMessage(), 422);
        }
    }

    public function totalSales()
    {
        try {
            return Order::where('payment_status', PaymentStatus::PAID)->sum('total');
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
            throw new Exception($exception->getMessage(), 422);
        }
    }

    public function totalTips()
    {
        try {
            return Order::where('payment_status', PaymentStatus::PAID)->sum('tip_amount');
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
            throw new Exception($exception->getMessage(), 422);
        }
    }

    public function totalOrders()
    {
        try {
            return Order::where('status', OrderStatus::DELIVERED)->count();
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
            throw new Exception($exception->getMessage(), 422);
        }
    }

    public function totalCustomers()
    {
        try {
            return User::role(EnumRole::CUSTOMER)->count();
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
            throw new Exception($exception->getMessage(), 422);
        }
    }

    public function totalMenuItems()
    {
        try {
            return Item::count();
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
            throw new Exception($exception->getMessage(), 422);
        }
    }

    public function paymentBreakdown(Request $request)
    {
        try {
            $order = new Order;
            
            // Determine date range
            if ($request->date_range === 'custom' && $request->start_date && $request->end_date) {
                $first_date = Carbon::parse($request->start_date)->startOfDay();
                $last_date = Carbon::parse($request->end_date)->endOfDay();
            } else {
                switch ($request->date_range) {
                    case 'today':
                        $first_date = Carbon::today()->startOfDay();
                        $last_date = Carbon::today()->endOfDay();
                        break;
                    case 'this_week':
                        $first_date = Carbon::now()->startOfWeek();
                        $last_date = Carbon::now()->endOfWeek();
                        break;
                    case 'this_month':
                        $first_date = Carbon::now()->startOfMonth();
                        $last_date = Carbon::now()->endOfMonth();
                        break;
                    case 'this_year':
                        $first_date = Carbon::now()->startOfYear();
                        $last_date = Carbon::now()->endOfYear();
                        break;
                    default:
                        throw new Exception('Invalid date range specified');
                }
            }

            if (!$first_date || !$last_date) {
                throw new Exception('Invalid date range');
            }

            // Calculate totals
            $total_sales = Order::where('payment_status', PaymentStatus::PAID)
                ->whereBetween('order_datetime', [$first_date, $last_date])
                ->sum('total');

            $total_tips = Order::where('payment_status', PaymentStatus::PAID)
                ->whereBetween('order_datetime', [$first_date, $last_date])
                ->sum('tip_amount');

            $online_sales = Order::where('payment_status', PaymentStatus::PAID)
                ->where('pos_payment_method', PosPaymentMethod::ONLINE_UPI)
                ->whereBetween('order_datetime', [$first_date, $last_date])
                ->sum('total');

            $cash_sales = Order::where('payment_status', PaymentStatus::PAID)
                ->where('pos_payment_method', PosPaymentMethod::CASH)
                ->whereBetween('order_datetime', [$first_date, $last_date])
                ->sum('total');

            // Daily breakdown
            $dates = [];
            $online_amounts = [];
            $cash_amounts = [];
            $tip_amounts = [];

            $current = $first_date->copy();
            while ($current <= $last_date) {
                $dates[] = $current->format('Y-m-d');
                
                $dayStart = $current->copy()->startOfDay();
                $dayEnd = $current->copy()->endOfDay();

                $online_amounts[] = (float) (Order::query()
                    ->where('payment_status', PaymentStatus::PAID)
                    ->where('pos_payment_method', PosPaymentMethod::ONLINE_UPI)
                    ->whereBetween('order_datetime', [
                        $dayStart->format('Y-m-d H:i:s'),
                        $dayEnd->format('Y-m-d H:i:s')
                    ])
                    ->sum('total') ?? 0);

                $cash_amounts[] = (float) (Order::query()
                    ->where('payment_status', PaymentStatus::PAID)
                    ->where('pos_payment_method', PosPaymentMethod::CASH)
                    ->whereBetween('order_datetime', [
                        $dayStart->format('Y-m-d H:i:s'),
                        $dayEnd->format('Y-m-d H:i:s')
                    ])
                    ->sum('total') ?? 0);
                
                $tip_amounts[] = (float) (Order::query()
                    ->where('payment_status', PaymentStatus::PAID)
                    ->whereBetween('order_datetime', [
                        $dayStart->format('Y-m-d H:i:s'),
                        $dayEnd->format('Y-m-d H:i:s')
                    ])
                    ->sum('tip_amount') ?? 0);

                $current->addDay();
            }

            return [
                'total_sales' => (float)$total_sales,
                'total_tips' => (float)$total_tips,
                'online_sales' => (float)$online_sales,
                'cash_sales' => (float)$cash_sales,
                'dates' => $dates,
                'online_amounts' => $online_amounts,
                'cash_amounts' => $cash_amounts,
                'tip_amounts' => $tip_amounts
            ];

        } catch (Exception $exception) {
            Log::info($exception->getMessage());
            throw new Exception($exception->getMessage(), 422);
        }
    }

    public function orderBreakdown(Request $request)
    {
        try {
            // Determine date range
            if ($request->date_range === 'custom' && $request->start_date && $request->end_date) {
                $first_date = Carbon::parse($request->start_date)->startOfDay();
                $last_date = Carbon::parse($request->end_date)->endOfDay();
            } else {
                switch ($request->date_range) {
                    case 'today':
                        $first_date = Carbon::today()->startOfDay();
                        $last_date = Carbon::today()->endOfDay();
                        break;
                    case 'this_week':
                        $first_date = Carbon::now()->startOfWeek();
                        $last_date = Carbon::now()->endOfWeek();
                        break;
                    case 'this_month':
                        $first_date = Carbon::now()->startOfMonth();
                        $last_date = Carbon::now()->endOfMonth();
                        break;
                    case 'this_year':
                        $first_date = Carbon::now()->startOfYear();
                        $last_date = Carbon::now()->endOfYear();
                        break;
                    default:
                        throw new Exception('Invalid date range specified');
                }
            }

            if (!$first_date || !$last_date) {
                throw new Exception('Invalid date range');
            }

            // Get base query for orders in date range
            $baseQuery = Order::query()
                ->where('status', '!=', OrderStatus::CANCELED)
                ->whereBetween('order_datetime', [
                    $first_date->format('Y-m-d H:i:s'),
                    $last_date->format('Y-m-d H:i:s')
                ]);

            // Calculate totals
            $total_orders = (int) ($baseQuery->count() ?? 0);
            $dine_in_orders = (int) ($baseQuery->clone()->where('order_method', 1)->count() ?? 0);
            $delivery_orders = (int) ($baseQuery->clone()->where('order_method', 3)->count() ?? 0);
            $take_away_orders = (int) ($baseQuery->clone()->where('order_method', 2)->count() ?? 0);

            // Get daily counts for the chart
            $dates = [];
            $dine_in_counts = [];
            $delivery_counts = [];
            $take_away_counts = [];

            $current = $first_date->copy();
            while ($current <= $last_date) {
                $dates[] = $current->format('Y-m-d');
                
                $dayStart = $current->copy()->startOfDay();
                $dayEnd = $current->copy()->endOfDay();

                $dayQuery = Order::query()
                    ->where('status', '!=', OrderStatus::CANCELED)
                    ->whereBetween('order_datetime', [
                        $dayStart->format('Y-m-d H:i:s'),
                        $dayEnd->format('Y-m-d H:i:s')
                    ]);

                $dine_in_counts[] = (int) ($dayQuery->clone()->where('order_method', 1)->count() ?? 0);
                $delivery_counts[] = (int) ($dayQuery->clone()->where('order_method', 3)->count() ?? 0);
                $take_away_counts[] = (int) ($dayQuery->clone()->where('order_method', 2)->count() ?? 0);

                $current->addDay();
            }

            return [
                'total_orders' => $total_orders,
                'dine_in_orders' => $dine_in_orders,
                'delivery_orders' => $delivery_orders,
                'take_away_orders' => $take_away_orders,
                'dates' => $dates,
                'dine_in_counts' => $dine_in_counts,
                'delivery_counts' => $delivery_counts,
                'take_away_counts' => $take_away_counts
            ];

        } catch (Exception $exception) {
            Log::info($exception->getMessage());
            throw new Exception($exception->getMessage(), 422);
        }
    }

    public function paymentMethodBreakdown($dateRange, $startDate = null, $endDate = null)
    {
        try {
            Log::info('Payment Method Breakdown called with:', [
                'dateRange' => $dateRange,
                'startDate' => $startDate,
                'endDate' => $endDate
            ]);
            
            $dates = $this->getDateRange($dateRange, $startDate, $endDate);
            $startDate = $dates['start_date'];
            $endDate = $dates['end_date'];
            
            Log::info('Date range processed:', [
                'startDate' => $startDate,
                'endDate' => $endDate
            ]);

            // Define order method constants if not already defined in the Order model
            $ORDER_METHOD_DINE_IN = 1;
            $ORDER_METHOD_TAKEAWAY = 2;
            $ORDER_METHOD_DELIVERY = 3;

            $query = Order::where('status', '!=', OrderStatus::CANCELED)
                ->whereBetween('order_datetime', [$startDate, $endDate]);
            
            Log::info('Base query created with conditions');

            $dineIn = (clone $query)->where('order_method', $ORDER_METHOD_DINE_IN)
                ->sum('total');

            $takeaway = (clone $query)->where('order_method', $ORDER_METHOD_TAKEAWAY)
                ->sum('total');

            $delivery = (clone $query)->where('order_method', $ORDER_METHOD_DELIVERY)
                ->sum('total');
            
            Log::info('Calculated payment totals:', [
                'dineIn' => $dineIn,
                'takeaway' => $takeaway,
                'delivery' => $delivery
            ]);

            return [
                'dine_in' => $dineIn,
                'takeaway' => $takeaway,
                'delivery' => $delivery
            ];
        } catch (\Exception $e) {
            Log::error('Error in paymentMethodBreakdown:', [
                'message' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
            throw new \Exception('Failed to fetch payment method breakdown data: ' . $e->getMessage());
        }
    }

    private function getDateRange($dateRange, $startDate = null, $endDate = null)
    {
        // Determine date range
        if ($dateRange === 'custom' && $startDate && $endDate) {
            $first_date = Carbon::parse($startDate)->startOfDay();
            $last_date = Carbon::parse($endDate)->endOfDay();
        } else {
            switch ($dateRange) {
                case 'today':
                    $first_date = Carbon::today()->startOfDay();
                    $last_date = Carbon::today()->endOfDay();
                    break;
                case 'this_week':
                    $first_date = Carbon::now()->startOfWeek();
                    $last_date = Carbon::now()->endOfWeek();
                    break;
                case 'this_month':
                    $first_date = Carbon::now()->startOfMonth();
                    $last_date = Carbon::now()->endOfMonth();
                    break;
                case 'this_year':
                    $first_date = Carbon::now()->startOfYear();
                    $last_date = Carbon::now()->endOfYear();
                    break;
                default:
                    throw new Exception('Invalid date range specified');
            }
        }

        if (!$first_date || !$last_date) {
            throw new Exception('Invalid date range');
        }

        return [
            'start_date' => $first_date->format('Y-m-d H:i:s'),
            'end_date' => $last_date->format('Y-m-d H:i:s')
        ];
    }
}