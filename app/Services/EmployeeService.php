...
class EmployeeService
{
    ...
    public $blockRoles = [EnumRole::ADMIN, EnumRole::CUSTOMER, EnumRole::DELIVERY_BOY, EnumRole::WAITER];
    ...
    public function posList()
    {
        try {
            return User::whereHas('roles', function ($query) {
                $query->whereNotIn('id', [EnumRole::ADMIN, EnumRole::CUSTOMER, EnumRole::DELIVERY_BOY]);
            })->where('status', Ask::YES)->get(['id', 'name']);
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
            throw new Exception($exception->getMessage(), 422);
        }
