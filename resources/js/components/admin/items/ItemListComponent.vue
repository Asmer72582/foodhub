<template>
        ...
        <div class="db-card">
            <div class="db-card-header border-none">
                ...
                <div class="db-card-filter">
                    <button v-if="permissionChecker('items_edit') && items && items.length > 0" 
                            class="db-btn py-2 text-white bg-primary mr-2" 
                            @click="openSequenceModal">
                        <i class="lab lab-sort-line lab-font-size-16"></i>
                        <span>{{ $t('button.reorder') }}</span>
                    </button>
            ...
            <div class="table-filter-div">
                <form class="p-4 sm:p-5 mb-5" @submit.prevent="search">
                    <div class="row">
                        ...
                        <div class="col-12 sm:col-6 md:col-4 xl:col-3">
                            <label for="searchIsFeatured" class="db-field-title after:hidden">{{$t("label.is_featured")}}</label>
            ...
            <div class="db-table-responsive">
                <table class="db-table stripe" id="print" :dir="direction">
                    ...
                    <template v-if="!loading.isActive">
                        <tbody v-if="!items || items.length === 0" class="db-table-body">
                            <tr class="db-table-body-tr">
                                <td class="db-table-body-td text-center" colspan="5">{{ $t('label.no_data_found') }}</td>
                            </tr>
                        </tbody>
                        <tbody v-else class="db-table-body">
                            <tr class="db-table-body-tr" v-for="item in items" :key="item.id">
                                <td class="db-table-body-td">{{ textShortener(item.name, 40) }}</td>
    ...
    <!-- Add the SequenceSelectionModal component -->
    <SequenceSelectionModal 
        modalId="sequenceModal" 
        :items="items" 
        :searchParams="props.search" 
        @sequence-updated="onSequenceUpdated" 
    />
</template>
...
import SequenceSelectionModal from './SequenceSelectionModal.vue';
...
export default {
    ...
    methods: {
        ...
        openSequenceModal: function() {
            // Open the sequence selection modal
            document.querySelector('#sequenceModal').classList.add('active');
            document.body.classList.add('overflow-hidden');
        },
        onSequenceUpdated: function() {
            // Refresh the list after sequence update
            this.list();
        },
        // Keeping this method for compatibility but it's no longer used directly
        sortItem: function () {
            // This method is kept for compatibility but is no longer used directly
            // The sorting is now handled by the SequenceSelectionModal component
            console.log('sortItem method is deprecated, use SequenceSelectionModal instead');
        },
        destroy: function (id) {
