            ...
            <div class="modal-body">
                ...
                <div v-if="order.order_method === orderMethodEnum.DELIVERY"
                    ...
                    <div class="pl-2 space-y-1">
                        <!-- Phone -->
                        <p v-if="order.user && order.user.phone" class="text-xs leading-5 text-heading flex items-start">
                            <span>Phone Number: {{ order.user.country_code }}{{ order.user.phone }}</span>
                        </p>
                        <!-- Address -->
                        <p v-if="order.user && order.user.address" class="text-xs leading-5 text-heading flex items-start">
                            <span class="flex-1">Address: {{ order.user.address }}</span>
                        </p>
                    </div>
                </div>
...
export default {
    ...
    computed: {
        ...
        deliveryCustomer() {
            if (this.order && this.order.order_method === this.orderMethodEnum.DELIVERY) {
                console.log('Full Order Object:', this.order);
                return {
                    name: this.order.user?.name || '',
                    phone: this.order.user?.phone || '',
                    address: this.order.order_address?.address || ''
                };
            }
            return null;
        }
