    ...
    <div class="order-card shadow-sm" :class="cardBorderClass">
        ...
        <div class="card-header-compact">
            <div class="flex items-center justify-between gap-1 mb-1">
                ...
                <div class="flex items-center gap-1.5">
                    <span v-if="order.order_type" :title="orderTypeLabel">
                        <i :class="orderTypeIcon" class="text-gray-400 text-[10px]"></i>
                    </span>
        ...
        <div class="card-body-compact">
            <div v-if="order.order_items && order.order_items.length > 0">
                <div v-for="(item, i) in order.order_items" :key="i" class="flex items-start justify-between py-0.5 leading-tight">
                    <span class="text-[12px] text-gray-800 font-medium">
                        <span class="text-primary font-bold mr-1">{{ item.quantity }}x</span>
                        {{ item.order_item ? item.order_item.name : 'Unknown' }}
                    </span>
                </div>
            </div>
