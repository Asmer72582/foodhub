        ...
        changeAddon: function (addon) {
            if (typeof this.addons[addon.id] === "undefined") {
                this.addons[addon.id] = {
                    ...
                    discount: 0,
                    currency_price: this.currencyFormat(
                        addon.offer.length > 0 ? addon.offer[0].convert_price : addon.convert_price,
                        this.setting.site_digit_after_decimal_point,
                        this.setting.site_default_currency_symbol,
                        this.setting.site_currency_position
                    ),
                    convert_price: addon.offer.length > 0 ? addon.offer[0].convert_price : addon.convert_price,
                    item_variations: {
                        variations: item.item_variations || {},
                        names: item.item_variation_names || {}
                    },
                    item_extras: {
                        extras: item.item_extras || [],
                        names: item.item_extra_names || []
                    },
