<template>
    ...
    <div class="mb-9">
        ...
        <div class="row">
            ...
            <div class="col-12 sm:col-6 xl:col-3">
                <div class="p-4 rounded-lg flex items-center gap-4 bg-[#FF9F43]">
                    <div class="w-12 h-12 rounded-full flex items-center justify-center bg-white">
                        <i class="lab lab-total-menu-items lab-font-size-24 text-[#FF9F43]"></i>
                    </div>
                    <div>
                        <h3 class="font-medium text-white">Total Tips</h3>
                        <h4 class="font-semibold text-[22px] leading-[34px] text-white">{{ total_tips }}</h4>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        totalTips: function () {
            this.loading.isActive = true;
            this.$store.dispatch("dashboard/totalTips").then((res) => {
                this.total_tips = res.data.data.total_tips;
                this.loading.isActive = false;
            }).catch((err) => {
                this.loading.isActive = false;
            });
